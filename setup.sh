...
python -c "import torch; print("請確認是否與git clone前安裝的版本相同" ,torch.__version__, torch.cuda.is_available())"

cd ~/Trustworthy-AI-HW-1
hf download jingyaogong/minimind-3 --local-dir ~/Trustworthy-AI-HW-1/minimind-3
python eval_tmmluplus.py eval --model_path ./minimind-3 --limit 20 --output smoke.json
